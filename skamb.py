...
NORMAL_TIMES = [
    Sound(8, 0), Sound(8, 45), Sound(8, 50), Sound(
        9, 35), Sound(9, 45), Sound(10, 30),
    Sound(10, 35), Sound(11, 20), Sound(12, 0), Sound(
        12, 45), Sound(12, 50), Sound(13, 35),
    Sound(13, 45), Sound(14, 30), Sound(14, 35), Sound(
        15, 20), Sound(15, 30), Sound(16, 15)
]

SHORTENED_TIMES = [
    Sound(8, 0), Sound(8, 30), Sound(8, 35), Sound(
        9, 5), Sound(9, 10), Sound(9, 40),
    Sound(9, 45), Sound(10, 15), Sound(10, 20), Sound(
        10, 50), Sound(10, 55), Sound(11, 25),
...
class BellScheduler:
    def __init__(self, master):
        ...
        for i, day in enumerate(self.lesson_times.keys()):
            ...
            var = StringVar()
            rb1 = Radiobutton(self.master, text="Normal", variable=var, value="Normal",
                              command=lambda day=day: self.set_lesson_times(day, var.get()))
            rb1.grid(row=i, column=1, padx=10, pady=10)
            rb2 = Radiobutton(self.master, text="Shortened", variable=var, value="Shortened",
                              command=lambda day=day: self.set_lesson_times(day, var.get()))
            rb2.grid(row=i, column=2, padx=10, pady=10)
            self.radio_buttons[day] = (rb1, rb2)

        # create button to start the schedule
        self.uploud_button = Button(
            self.master, text="Uploud sound", command=self.choose_sound_file)
        ...
        self.file_name_label.grid(
            row=len(self.lesson_times)+1, column=1, pady=10)

        # create button to start the schedule
        self.start_button = Button(
            self.master, text="Start Schedule", command=self.start_scheduler)
        self.start_button.grid(row=len(self.lesson_times)+2, column=1, pady=10)

        # create button to start the schedule
        self.stop_button = Button(
            self.master, text="Stop Schedule", command=self.stop_scheduler)
    ...
    def run_scheduler(self):
        """Run the bell scheduler."""
        while self.scheduler_running:
            now = datetime.datetime.now()
            day = now.strftime("%A")
            lesson_times = self.lesson_times[day]
            next_bell_time = None
            for i, sound in enumerate(lesson_times):
                bell_time = datetime.datetime(
                    now.year, now.month, now.day, sound.hour, sound.minute)
                if now <= bell_time:
                    next_bell_time = bell_time
                    time_until_bell = (next_bell_time - now).total_seconds()
                    break
            if next_bell_time:
                time.sleep(time_until_bell)
                self.play_bell_sound(self.bell_sound_file)
            else:
                self.scheduler_running = False
                messagebox.showinfo(
                    "Bell Scheduler", "Lesson times for {} are over.".format(day))
                self.stop_scheduler()
